...
function adicionarnot() {
    ...
    if (edit < 0) {
        if (edit < 0 && titulo == '' || data == '' || descricao == '' || autor == '' || new Date(data) < new Date(atual) || categoria == '') {
            if (titulo == '') {
    ...
    if (edit > -1) {
        if (edit > -1 && titulo == '' || data == '' || descricao == '' || autor == '' || new Date(data) < new Date(atual) || categoria == '') {
            if (titulo == '') {
                alert('preencha o campo "titulo"')
        ...
        if (sac.toUpperCase() == 'NEW') {
            text[i - 3] = `<section class="noticiToDo" id="noticia${i - 3}"><div class="separator"><h1 class="titulocss">Titulo: ${titulos[i - 3]}</h1><h2 class="titlesno">Categoria: ${categorias[i - 3]}</h2><h6 class="dategera">Data: ${dias[i - 3]}/${meses[i - 3]}/${anos[i - 3]}</h6><p class="titlesno">Descrição: ${descricoes[i - 3]}</p><h4 class="titlesno">Autor: ${autores[i - 3]}</h4><div class="flexin"><button class="butdel" onclick="deletar(${i - 3})">Deletar 🗑️</button><button class="butedit" onclick="editar(${i - 3})">Editar ✏️</button> <p class="rightjo" id="new${i - 3}">new</p></div></div></section>`
        }
        ...
        text[edit] = `<section class="noticiToDo" id="noticia${edit}"><div class="separator"><h1 class="titulocss">Titulo: ${titulos[edit]}</h1><h2 class="titlesno">Categoria: ${categorias[edit]}</h2><h6 class="dategera">Data: ${dias[edit]}/${meses[edit]}/${anos[edit]}</h6><p class="titlesno">Descrição: ${descricoes[edit]}</p><h4 class="titlesno">Autor: ${autores[edit]}</h4><div class="flexin"><button class="butdel" onclick="deletar(${edit})">Deletar 🗑️</button><button class="butedit" onclick="editar(${edit})">Editar ✏️</button><p class="rightjo" id="new${edit}">edited</p></div></div></section>`
        //adicionar outros textos
        document.getElementById(`todos`).innerHTML = `${text.join(' ')}`
...
function editar(y) {
    ...
    text[y] = `<section class="noticiToDo" id="noticia${y}"><div class="separator"><h1 class="titulocss">Titulo: ${titulos[y]}</h1><h2 class="titlesno">Categoria: ${categorias[y]}</h2><h6 class="dategera">Data: ${dias[y]}/${meses[y]}/${anos[y]}</h6><p class="titlesno">Descrição: ${descricoes[y]}</p><h4 class="titlesno">Autor: ${autores[y]}</h4><div class="flexin"><button class="butdel" onclick="deletar(${y})">Deletar 🗑️</button><button class="butedit" onclick="editar(${y})">Editar ✏️</button> <p class="rightjo" id="new${y}">edited</p></div></div></section>`
}
